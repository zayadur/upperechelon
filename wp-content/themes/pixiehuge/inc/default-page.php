<div class="container split-flex">
	<section id="single-page">
				...
				if(get_the_tags()):
					...
					if ($posttags) {
						foreach($posttags as $tag) {
							...
							echo '<a href="' . esc_url($tagUrl) . '">' . $tag->name . '</a>';
						}
					}
		...
		<?php
		$defaults = array(
			'before'           => '<p>' . esc_html__( 'Pages:', 'pixiehuge'),
			'after'            => '</p>',
			'link_before'      => '',
			'link_after'       => '',
			'next_or_number'   => 'number',
			'separator'        => ' ',
			'nextpagelink'     => esc_html__( 'Next page', 'pixiehuge'),
			'previouspagelink' => esc_html__( 'Previous page', 'pixiehuge'),
			'pagelink'         => '%',
			'echo'             => 1
		);
